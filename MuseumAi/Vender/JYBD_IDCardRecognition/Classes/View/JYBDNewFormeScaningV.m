...
- (void)creatUI
{
    ...
    self.scanLab.frame = CGRectMake(15, 5, 20, self.frame.size.height);
    
    self.bgImageView.frame = CGRectMake(45, 0, self.frame.size.width-100, (self.frame.size.width-100)*1.57);
    ...
    self.turnOnOrOffView.frame = CGRectMake(self.bounds.size.width-50, 80*CustomScreenFit, 50, self.frame.size.height);
    CGSize size = [self getLabelSizeWithStr:self.turnOnOrOffLab.text font:self.turnOnOrOffLab.font];
    
    self.turnOnOrOffBtn.frame = CGRectMake(0, 0, 70, 50);
...
- (UIImageView *)bgImageView
{
    if (_bgImageView == nil)
    {
        ...
        if (self.isBehinded) {
            imag = [UIImage imageNamed:@"scaningbgimage2" inBundle:[IDCardRectManager getImageBundle] compatibleWithTraitCollection:nil];
        }
...
- (void)setIsBehinded:(BOOL)isBehinded{
    ...
    if (_isBehinded) {
        _bgImageView.image= [UIImage imageNamed:@"scaningbgimage2" inBundle:[IDCardRectManager getImageBundle] compatibleWithTraitCollection:nil];;
    }else{
        _bgImageView.image= [UIImage imageNamed:@"scaningbgimage" inBundle:[IDCardRectManager getImageBundle] compatibleWithTraitCollection:nil];;
    }
}
...
- (UIButton *)turnOnOrOffBtn
{
    if (_turnOnOrOffBtn == nil)
    {
        ...
        [_turnOnOrOffBtn setBackgroundImage:nil forState:UIControlStateNormal];
...
- (UIView *)turnOnOrOffView
{
    if (_turnOnOrOffView == nil)
    {
        ...
        _turnOnOrOffView.userInteractionEnabled = YES;
    }
