...
static const CGFloat YMSNavigationBarOriginalTopSpace = 20.0;
static const CGFloat YMSPhotoFetchScaleResizingRatio = 0.75;

@interface YMSPhotoPickerViewController ()<UICollectionViewDataSource, UICollectionViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, PHPhotoLibraryChangeObserver, UIActionSheetDelegate>
...
- (void)viewDidLoad
{
    ...
    if (@available(iOS 11.0, *)) {
        self.photoCollectionView.contentInsetAdjustmentBehavior = UIScrollViewContentInsetAdjustmentNever;//UIScrollView也适用
    }else {
    ...
    self.navigationBarTopLayoutConstraint.constant = 20;
...
- (BOOL)collectionView:(UICollectionView *)collectionView shouldSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!self.canAddPhoto
        || cell.isSelected) {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:[NSString stringWithFormat:@"最多只能选择%ld张照片", (unsigned long)self.numberOfPhotoToSelect] message:@"" delegate:nil cancelButtonTitle:@"我知道了" otherButtonTitles:nil, nil];
        [alertView show];
...
{
    return YES;
//    return (self.numberOfPhotoToSelect < 10);
}
