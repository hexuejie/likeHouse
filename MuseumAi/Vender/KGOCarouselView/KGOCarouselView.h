...
/** 代理 */
@protocol KGOCarouselViewDelegate <NSObject>
@optional
/** 
 ...
 * @param index         图片索引
 */
-(void)carouselView:(KGOCarouselView *)carouselView clickImageAtIndex:(NSInteger)index;
@end
...
/**
 * 说明：想要正常使用，图片数组imageArray必须设置
 * 控件的frame必须设置，xib/storyboard创建的可不舍之
 * 其他属性都有默认值，可不设置
 */
@interface KGOCarouselView : UIView
#pragma mark 属性
/** 
 ...
 * 代理，用来处理图片的点击
 */
@property (nonatomic, weak) id<KGOCarouselViewDelegate> delegate;
 ...
 * 不设置则为系统默认
 *
 * @param pageImage     其他页码的颜色
 * @param currentImage  当前页码的颜色
 */
