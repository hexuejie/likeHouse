...
@interface KGOCarouselView()<UIScrollViewDelegate>
/** 轮播的图片数组 */
...
static NSTimeInterval defaultTime = 5;
static CGFloat horMargin = 10;
static CGFloat verMargin = 5;

@implementation KGOCarouselView
#pragma mark - 初始化方法
//创建用来缓存图片的文件夹
+(void)initialize
{
    cache = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"KGOCarousel"];
    BOOL isDir = NO;
...
-(UIScrollView *)scrollView
{
    if(!_scrollView)
    {
        ...
        _currentTitleLb.textColor = [UIColor whiteColor];
        _currentTitleLb.font = [UIFont systemFontOfSize:14.0f];
        _currentTitleLb.backgroundColor = [[UIColor blackColor]colorWithAlphaComponent:0.2];
        [_currImageView addSubview:_currentTitleLb];
...
-(void)setImageArray:(NSArray *)imageArray
{
    ...
    if (self.titleArray.count != 0 &&
        self.titleArray.count == self.imageArray.count) {
        self.currentTitleLb.text = self.titleArray[_currIndex];
    } else {
...
-(void)setScrollViewContentSize
{
    if(_images.count > 1)
    {
        ...
        self.currentTitleLb.frame = CGRectMake(0, self.height - 30.0f, self.width, 30.0f);
...
-(void)downloadImages:(int)index
{
    ...
    NSBlockOperation *download = [NSBlockOperation blockOperationWithBlock:^{
        ...
        if(image)
        {
            ...
            //如果下载的图片未当前要显示的图片，直接到主线程给imageView赋值，否则要等到下一轮才会显示
            if(_currIndex == index)
            {
...
-(void)changeToNext
{
    ...
    if (self.titleArray.count != 0 &&
        self.titleArray.count == self.imageArray.count) {
        self.currentTitleLb.text = self.titleArray[self.currIndex];
    } else {
        self.currentTitleLb.hidden = YES;
    }
}
