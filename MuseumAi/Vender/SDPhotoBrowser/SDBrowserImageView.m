...
- (void)layoutSubviews
{
    ...
    for (UIView *view in self.subviews) {
        if ([view isKindOfClass:[UIImageView class]] && view.tag == TOPTAG) {
            [view removeFromSuperview];
        }
    }
    if (_topImageName != nil) {
        UIImageView *topImageView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:_topImageName]];
        topImageView.tag = TOPTAG;
        [self addSubview:topImageView];
        [self bringSubviewToFront:topImageView];
        [topImageView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.width.height.mas_equalTo(SCREEN_WIDTH-60.0f);
...
- (void)setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder topImageName:(NSString *)topImageName
{
